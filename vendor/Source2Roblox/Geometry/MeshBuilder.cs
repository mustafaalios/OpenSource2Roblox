...
namespace Source2Roblox.Geometry
{
    public static class MeshBuilder
    {
        ...
        private static string RobloxTexturePath(string sourcePath)
        {
            return Path.ChangeExtension(sourcePath, ".png").Replace('\\', '/');
        }

        private static void WaitForUploads(List<Task> uploadPool)
        {
            int total = uploadPool.Count;

            if (total == 0)
                return;

            int complete = uploadPool.Count(task => task.IsCompleted);
            Program.Emit("progress", "Uploading Roblox assets.", new
            {
                phase = "uploads",
                current = complete,
                total
            ...
            var pending = uploadPool.Where(task => !task.IsCompleted).ToList();
            int lastReported = complete;

            while (pending.Count > 0)
            {
                var finished = Task.WhenAny(pending).Result;
                pending.Remove(finished);
                finished.GetAwaiter().GetResult();
                complete++;

                if (complete == total || complete - lastReported >= Math.Max(1, total / 20))
                {
        ...
        public static Model BakeMDL_RBXM(ModelFile model, int skin = 0, string sourceRootDir = null)
        {
            ...
            foreach (var meshBuffer in meshBuffers)
            {
                ...
                if (!string.IsNullOrEmpty(diffusePath))
                    assetResolver.BindTexture(diffusePath, uploadPool, surface, "ColorMap");

                if (!string.IsNullOrEmpty(bumpPath))
                    assetResolver.BindTexture(bumpPath, uploadPool, surface, "NormalMap");
                ...
                if (vmt.EnvMap)
                {
                    assetResolver.BindTexture(ENV_DARK, uploadPool, surface, "RoughnessMap");
                    assetResolver.BindTexture(ENV_LIGHT, uploadPool, surface, "MetalnessMap");
                }
                else
                {
                    assetResolver.BindTexture(ENV_DARK, uploadPool, surface, "MetalnessMap");
                    assetResolver.BindTexture(ENV_LIGHT, uploadPool, surface, "RoughnessMap");
                }
            ...
            foreach (var meshPart in meshBinds.Keys)
            {
                ...
                assetResolver.BindMesh($"{meshDir}/{name}.mesh", uploadPool, meshPart, "MeshId");
            }
        ...
        public static WorldGeometry BakeBSP_RBXL(BSPFile bsp, GameMount game = null, string sourceRootDir = null, bool openWhenDone = false)
        {
            string mapName = bsp.Name;
            ...
            if (worldSpawn != null)
            {
                ...
                if (!string.IsNullOrEmpty(skyName))
                {
                    ...
                    foreach (var pair in sky.Properties)
                    {
                        ...
                        assetResolver.BindTexture(diffuse, uploadPool, prop);
            ...
            foreach (var cluster in clusters)
            {
                ...
                using (var meshStream = File.OpenWrite(meshPath))
                {
                    ...
                    assetResolver.BindMesh($"maps/{mapName}/{meshName}", uploadPool, meshPart, "MeshId");
                    ...
                    if (materialSets.TryGetValue(material, out var vmt))
                    {
                        ...
                        if (!string.IsNullOrEmpty(diffuse))
                        {
                            ...
                            assetResolver.BindTexture(diffuse, uploadPool, surface, "ColorMap");
                        }
                        
                        if (!string.IsNullOrEmpty(bump))
                        {
                            vmt.SaveVTF(bump, sourceDir, true);
                            assetResolver.BindTexture(bump, uploadPool, surface, "NormalMap");
                        }
            ...
            foreach (var request in modelSets)
            {
                ...
                skins[skin] = BakeMDL_RBXM(modelFile, skin, sourceRootDir);
            }
